...
import { getTodoIdsForMonth, isTodoInMonth } from "./utils/todoDates";
...
const App: React.FC = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 新しい画像の保存成功後にだけ古い別拡張子を削除する。
      const extensions = ['jpg', 'jpeg', 'png', 'webp', 'gif'];
      const avatarKeysToDelete = new Set(
        extensions
          .map((ext) => `users/${user.id}/avatar.${ext}`)
          .filter((key) => key !== uploadedKey)
      );
      if (user.avatarImageUrl && user.avatarImageUrl !== uploadedKey) {
        avatarKeysToDelete.add(user.avatarImageUrl);
      }
      for (const oldAvatarKey of avatarKeysToDelete) {
        try {
  ...
  const handleDeleteMonthTodos = async () => {
    ...
    const monthTodoIds = getTodoIdsForMonth(todos, year, month);
    ...
    // Supabaseで一括削除
    const success = await deleteMonthTodos(monthTodoIds);
    if (!success) {
      // 失敗したら元に戻す
      setTodos((prev) => [...prev, ...monthTodos]);
      alert("月のTodo削除に失敗しました");
    } else {
      // DB削除が成功した後にだけR2から画像を削除する。
      if (totalImages > 0) {
