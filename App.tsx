...
const App: React.FC = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 新規アップロード成功後に、同じキーを除外して古い候補だけ掃除する
      const extensions = ['jpg', 'jpeg', 'png', 'webp', 'gif'];
      const oldAvatarKeys = new Set(extensions.map((ext) => `users/${user.id}/avatar.${ext}`));
      if (user.avatarImageUrl) {
  ...
  const handleSetDateColor = async (dateStr: string, color: DateColorType) => {
    ...
    // 楽観的更新
    setDateColors((prev) => applyOptimisticDateColor(prev, dateStr, color, user.id));
  ...
  const handleSetDateLabel = async (dateStr: string, label: string | null) => {
    ...
    setDateColors((prev) => applyOptimisticDateLabel(prev, dateStr, label, user.id));
  ...
  const handleDeleteMonthTodos = async () => {
    ...
    const monthTodos = todos.filter((todo) => isDateStrInMonth(todo.dateStr, year, month));
