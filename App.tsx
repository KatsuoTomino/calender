...
import { formatLocalDate, isDateInMonth } from "./utils/todoDates";
...
const App: React.FC = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.log("🖼️ アップロードしたアバター画像の表示URLを取得中:", uploadedKey);
      ...
      // 新しい画像の保存が成功してから旧キーを掃除し、失敗時に旧アバターを失わないようにする。
      const extensions = ['jpg', 'jpeg', 'png', 'webp', 'gif'];
      const oldAvatarKeys = new Set([
        ...extensions.map((ext) => `users/${user.id}/avatar.${ext}`),
        ...(user.avatarImageUrl && !user.avatarImageUrl.startsWith("http") ? [user.avatarImageUrl] : []),
      ]);
  ...
  const handleDeleteMonthTodos = async () => {
    ...
    const monthTodos = todos.filter((todo) => isDateInMonth(todo.dateStr, year, month));
    ...
    if (!success) {
      ...
      // DB削除の成功後にR2を掃除する。失敗してもTodoの画像参照は既に消えているため、データ損失にはしない。
      if (totalImages > 0) {
