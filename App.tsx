...
import { AVATAR_EXTENSIONS, normalizeR2Key } from "./utils/r2Keys";
import { isTodoDateInMonth } from "./utils/todoDates";
...
const App: React.FC = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const oldAvatarKey of oldAvatarKeys) {
        if (normalizeR2Key(oldAvatarKey) === uploadedKey) continue;
        try {
  ...
  const handleDeleteMonthTodos = async () => {
    ...
    const monthTodos = todos.filter((todo) =>
      isTodoDateInMonth(todo.dateStr, year, month)
    );
    ...
    if (!success) {
      ...
      // DB削除成功後にR2をクリーンアップする。失敗してもDB参照は残らないためデータ損失にはならない。
      if (totalImages > 0) {
