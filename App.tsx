...
import { R2_IMAGE_EXTENSIONS } from "./utils/r2Keys";
import { isDateInMonthString } from "./utils/todoDates";
...
const App: React.FC = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const obsoleteAvatarKeys = R2_IMAGE_EXTENSIONS
        .map((ext) => `users/${user.id}/avatar.${ext}`)
        .filter((key) => key !== uploadedKey);
      if (user.avatarImageUrl && user.avatarImageUrl !== uploadedKey) {
        obsoleteAvatarKeys.push(user.avatarImageUrl);
      }

      for (const oldAvatarKey of [...new Set(obsoleteAvatarKeys)]) {
        try {
          await deleteImageFromR2(oldAvatarKey);
        } catch (error) {
          console.warn("古いアバター画像の削除に失敗:", error);
        }
      }
    } catch (error) {
  ...
  const handleDeleteMonthTodos = async () => {
    ...
    const monthTodos = todos.filter((todo) => {
      return isDateInMonthString(todo.dateStr, year, month);
    });
    ...
    if (!deletedTodos) {
      ...
      if (deletedTodos.length !== monthTodos.length) {
        await loadTodos();
      }
      const deletedImages = deletedTodos.flatMap((todo) => todo.imageUrls || []);
      if (deletedImages.length > 0) {
        console.log(`🗑️ 月の削除に伴い、${deletedImages.length}枚の画像をR2から削除中...`);
