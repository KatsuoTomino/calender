...
import { AVATAR_IMAGE_EXTENSIONS } from "./utils/r2Keys";
...
const App: React.FC = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const oldAvatarKeys = new Set<string>(
        AVATAR_IMAGE_EXTENSIONS.map((ext) => `users/${user.id}/avatar.${ext}`)
      );
      if (user.avatarImageUrl) {
      ...
      for (const oldAvatarKey of oldAvatarKeys) {
        ...
        try {
          ...
          console.warn("古いアバター画像の削除に失敗しました:", oldAvatarKey, error);
        }
      }
    } catch (error) {
  ...
  const handleSetDateColor = async (dateStr: string, color: DateColorType) => {
    ...
    // 楽観的更新
    setDateColors((prev) =>
      applyDateColorUpdate(prev, dateStr, color, user.id, () => crypto.randomUUID())
    );
  ...
  const handleSetDateLabel = async (dateStr: string, label: string | null) => {
    ...
    setDateColors((prev) =>
      applyDateLabelUpdate(prev, dateStr, label, user.id, () => crypto.randomUUID())
    );
  ...
  const handleDeleteMonthTodos = async () => {
    ...
    const deletedTodoIds = await deleteTodosByIds(monthTodos.map((todo) => todo.id));
    if (!deletedTodoIds) {
      // 失敗したら元に戻す
    ...
    const deletedIdSet = new Set(deletedTodoIds);
