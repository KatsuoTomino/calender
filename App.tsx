...
const App: React.FC = () => {
  ...
  const handleSetDateColor = async (dateStr: string, color: DateColorType) => {
    ...
    // 楽観的更新
    setDateColors((prev) => applyDateColorOptimistic(prev, dateStr, color, user.id));
  ...
  const handleSetDateLabel = async (dateStr: string, label: string | null) => {
    ...
    setDateColors((prev) => applyDateLabelOptimistic(prev, dateStr, label, user.id));
