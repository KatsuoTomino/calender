...
import type { User, TodoItem, DateColor, DateColorType } from "./types";
...
import { applyDateColorOptimistic, applyDateLabelOptimistic } from "./utils/dateColors";
import { filterTodosForMonth } from "./utils/todoDates";
...
const App: React.FC = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const ext of extensions) {
        oldAvatarKeys.add(`users/${user.id}/avatar.${ext}`);
      }
      if (user.avatarImageUrl) {
  ...
  const handleSetDateColor = async (dateStr: string, color: DateColorType) => {
    ...
    // 楽観的更新
    setDateColors((prev) =>
      applyDateColorOptimistic(prev, dateStr, color, user.id, () => crypto.randomUUID())
    );
  ...
  const handleSetDateLabel = async (dateStr: string, label: string | null) => {
    ...
    setDateColors((prev) =>
      applyDateLabelOptimistic(prev, dateStr, label, user.id, () => crypto.randomUUID())
    );
  ...
  const handleDeleteMonthTodos = async () => {
    ...
    // Supabaseで一括削除
    const deletedTodos = await deleteMonthTodos(monthTodos.map((todo) => todo.id));
    ...
    const notDeletedTodos = monthTodos.filter((todo) => !deletedIds.has(todo.id));
    if (notDeletedTodos.length > 0) {
      setTodos((prev) => [...prev, ...notDeletedTodos]);
      alert("一部のTodoを削除できませんでした");
    }

    const deletedTodosWithImages = deletedTodos.filter(
      (todo) => todo.imageUrls && todo.imageUrls.length > 0
    );
