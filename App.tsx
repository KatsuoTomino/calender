...
const App: React.FC = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // アップロード成功後に古い画像だけ削除する（失敗時に既存アバターを失わないため）。
  ...
  const handleSetDateColor = async (dateStr: string, color: DateColorType) => {
    ...
    // 楽観的更新
    setDateColors((prev) => {
      return applyDateColorUpdate(prev, dateStr, color, user.id, () => crypto.randomUUID());
    });
  ...
  const handleSetDateLabel = async (dateStr: string, label: string | null) => {
    ...
    setDateColors((prev) => {
      return applyDateLabelUpdate(prev, dateStr, label, user.id, () => crypto.randomUUID());
    });
  ...
  const handleDeleteMonthTodos = async () => {
    ...
    const monthTodos = todos.filter((todo) => {
      return isTodoInMonth(todo, year, month);
    });
