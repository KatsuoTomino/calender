...
import { isDateStrInMonth } from "./utils/todoDates";
...
const App: React.FC = () => {
  ...
  const deleteImagesFromR2BestEffort = async (imageKeys: Array<string | null | undefined>) => {
  ...
  const cleanupOldAvatarImages = async (userId: string, uploadedKey: string, legacyKey?: string) => {
    const extensions = ["jpg", "jpeg", "png", "webp", "gif"];
    const oldKeys = new Set([
      ...extensions.map((ext) => `users/${userId}/avatar.${ext}`),
      legacyKey,
    ]);
    oldKeys.delete(uploadedKey);
    await deleteImagesFromR2BestEffort([...oldKeys]);
  };
  ...
  const handleDeleteTodo = async (id: string) => {
    ...
    if (success && deletedTodo?.imageUrls?.length) {
      await deleteImagesFromR2BestEffort(deletedTodo.imageUrls);
    }
  ...
  const handleDeleteMonthTodos = async () => {
    ...
    const monthTodos = todos.filter((todo) => {
      return isDateStrInMonth(todo.dateStr, year, month);
    });
    ...
    // Supabaseで一括削除
    const deletedIds = await deleteMonthTodos(monthTodos.map((todo) => todo.id));
    if (!deletedIds) {
      // 失敗したら元に戻す
      ...
      const deletedImageKeys = monthTodos
        .filter((todo) => deletedIdSet.has(todo.id))
        .flatMap((todo) => todo.imageUrls || []);
      if (deletedImageKeys.length > 0) {
        console.log(`🗑️ 月の削除に伴い、${deletedImageKeys.length}枚の画像をR2から削除中...`);
        await deleteImagesFromR2BestEffort(deletedImageKeys);
      }
