...
import { avatarKeysForUser, normalizeR2Key } from "./utils/r2Keys";
...
const App: React.FC = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const oldAvatarKeys = new Set([
        ...avatarKeysForUser(user.id),
        ...(user.avatarImageUrl ? [normalizeR2Key(user.avatarImageUrl)] : []),
      ]);
  ...
  const handleSetDateColor = async (dateStr: string, color: DateColorType) => {
    ...
    // 楽観的更新
    setDateColors((prev) => {
      return applyDateColorChange(prev, dateStr, color, user.id, () =>
        crypto.randomUUID()
      );
    });
  ...
  const handleSetDateLabel = async (dateStr: string, label: string | null) => {
    ...
    setDateColors((prev) => {
      return applyDateLabelChange(prev, dateStr, label, user.id, () =>
        crypto.randomUUID()
      );
    });
  ...
  const handleDeleteMonthTodos = async () => {
    ...
    const monthTodos = todos.filter((todo) => {
      return isTodoInMonth(todo.dateStr, year, month);
    });
    ...
    // Supabaseで、確認時点のIDだけを一括削除
    const success = await deleteMonthTodos(monthTodos.map((todo) => todo.id));
    if (!success) {
      // 失敗したら元に戻す
      setTodos((prev) => [...prev, ...monthTodos]);
      alert("月のTodo削除に失敗しました");
    } else {
      // DB削除が成功してからR2を掃除する。先にR2を消すと、DB失敗時に画像だけ失われる。
      if (totalImages > 0) {
