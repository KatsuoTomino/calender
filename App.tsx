...
const App: React.FC = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const oldAvatarKeys = ['jpg', 'jpeg', 'png', 'webp', 'gif']
      ...
      await Promise.allSettled(oldAvatarKeys.map((key) => deleteImageFromR2(key)));
  ...
  const handleSetDateColor = async (dateStr: string, color: DateColorType) => {
    ...
    // 楽観的更新
    setDateColors((prev) => {
      return applyDateColorUpdate(prev, dateStr, color, user.id);
    });
  ...
  const handleSetDateLabel = async (dateStr: string, label: string | null) => {
    ...
    setDateColors((prev) => {
      return applyDateLabelUpdate(prev, dateStr, label, user.id);
    });
  ...
  const handleDeleteMonthTodos = async () => {
    ...
    if (notDeletedTodos.length > 0) {
      ...
      alert("一部のTodoを削除できませんでした。画面を更新して再確認してください。");
    }

    const deletedTodosWithImages = todosWithImages.filter((todo) => deletedIdSet.has(todo.id));
    if (deletedTodosWithImages.length > 0) {
      console.log(`🗑️ 月の削除に伴い、画像をR2から削除中...`);
