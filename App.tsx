...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const { data: authListener } = onAuthStateChange(async (authUser) => {
      if (authUser) {
        ...
        const sameStoredUser = storedUser?.id === authUser.id ? storedUser : null;
        const appUser: User = {
          id: authUser.id,
          name: authUser.user_metadata?.name || "ユーザー",
          role: sameStoredUser?.role || "partner",
          avatarColor: sameStoredUser?.avatarColor || "bg-purple-500",
          avatarImageUrl: sameStoredUser?.avatarImageUrl,
        };
        setUser(appUser);
        saveUser(appUser);
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 新しいアバターの保存成功後に旧キーだけ削除し、失敗時の画像喪失を避ける。
  ...
  const handleDeleteMonthTodos = async () => {
    ...
    const startDateStr = formatLocalDate(new Date(year, currentDate.getMonth(), 1));
    const endDateStr = formatLocalDate(new Date(year, currentDate.getMonth() + 1, 0));
