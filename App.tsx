...
import { applyOptimisticDateColor, applyOptimisticDateLabel } from "./utils/dateColors";
import { R2_AVATAR_EXTENSIONS, buildAvatarKey, extractR2Key } from "./utils/r2Keys";
...
const App: React.FC = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const oldAvatarKeys = new Set<string>();
      for (const ext of R2_AVATAR_EXTENSIONS) {
        const oldAvatarKey = buildAvatarKey(user.id, ext);
        if (oldAvatarKey !== uploadedKey) {
          oldAvatarKeys.add(oldAvatarKey);
        }
      }
      if (user.avatarImageUrl) {
        const storedKey = extractR2Key(user.avatarImageUrl);
        if (storedKey !== uploadedKey) {
          oldAvatarKeys.add(storedKey);
        }
      }
      Promise.allSettled([...oldAvatarKeys].map((oldKey) => deleteImageFromR2(oldKey))).then(
        () => console.log("✅ 古いアバター画像のクリーンアップ完了")
      );
  ...
  const handleSetDateColor = async (dateStr: string, color: DateColorType) => {
    ...
    // 楽観的更新
    setDateColors((prev) => {
      return applyOptimisticDateColor(prev, dateStr, color, user.id, () => crypto.randomUUID());
    });
  ...
  const handleSetDateLabel = async (dateStr: string, label: string | null) => {
    ...
    setDateColors((prev) => {
      return applyOptimisticDateLabel(prev, dateStr, label, user.id, () => crypto.randomUUID());
    });
  ...
  const handleDeleteMonthTodos = async () => {
    ...
    const monthTodos = todos.filter((todo) => isTodoInMonth(todo.dateStr, year, month));
    ...
    // Supabaseで確認済みのIDだけを一括削除
    const deletedIds = await deleteTodosByIds(monthTodos.map((todo) => todo.id));
    if (!deletedIds) {
      // 失敗したら元に戻す
      ...
      const failedTodos = monthTodos.filter((todo) => !deletedIdSet.has(todo.id));
      if (failedTodos.length > 0) {
        setTodos((prev) => [...prev, ...failedTodos]);
        alert(`${failedTodos.length}件のTodoを削除できませんでした`);
      }

      const deletedTodosWithImages = todosWithImages.filter((todo) => deletedIdSet.has(todo.id));
      const deletedImageCount = deletedTodosWithImages.reduce(
        (sum, todo) => sum + (todo.imageUrls?.length || 0),
        0
      );
      if (deletedImageCount > 0) {
        console.log(`🗑️ 月の削除に伴い、${deletedImageCount}枚の画像をR2から削除中...`);
        for (const todo of deletedTodosWithImages) {
