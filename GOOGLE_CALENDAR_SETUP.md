...
3. 取り込み候補のタスク名一覧を確認し、問題なければ取り込む
...
- Gカレのチェックと event id はデータベースに保存され、PC / 携帯で共有されます
- チェックの付け外しはローカルの印だけで、Google 側の予定は作成・削除しません
- Gカレにチェックが付いたタスクを削除すると、Googleカレンダーの予定も一緒に削除します
- チェックを外しても event id は保持されます（再度チェック／再エクスポート用）。削除時に両方消すにはチェックを付けたままにしてください
