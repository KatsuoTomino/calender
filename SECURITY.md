...
- R2 のアクセスキーはサーバー専用の `R2_*` 環境変数に限定し、`VITE_*` として公開しない
...
# R2設定（Vercel Function / サーバー専用。VITE_ を付けない）
...
- 本番環境では環境変数を使用
- 定期的にキーをローテーション
- `VITE_*` 環境変数はクライアントのビルド成果物に含まれるため、シークレットには使用しない
- 過去に `VITE_R2_*` として設定した R2 キーは漏えい済みとみなし、Cloudflare 側でローテーションする
