...
Viteでは`VITE_`で始まる値がクライアントバンドルに含まれるため、R2のシークレットには`VITE_`を付けないでください。画像操作は認証済みの`/api/r2`サーバー関数経由で実行されます。
...
- 本番環境では環境変数を使用
- ブラウザに配信される`VITE_*`環境変数に秘密情報を入れない
- 定期的にキーをローテーション
