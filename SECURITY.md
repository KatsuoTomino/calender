...
- R2のシークレットは`R2_*`としてサーバー関数だけで使用し、`VITE_*`にはしない
...
Viteでは`VITE_`で始まる環境変数がクライアントバンドルに含まれるため、R2のシークレットには使用しないでください。
...
- 本番環境では環境変数を使用
- ブラウザに公開される`VITE_*`環境変数にシークレットを入れない
- 定期的にキーをローテーション
