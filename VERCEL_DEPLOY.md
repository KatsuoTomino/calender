...
- ✅ Cloudflare R2 環境変数（Account ID、Access Key、Secret、Bucket）
...
R2_ENDPOINT=your_r2_endpoint_optional
```
...
5. R2 の秘密情報は `VITE_` を付けずに追加（Vite公式ドキュメントでは `VITE_*` はクライアントへ露出するとされています）
...
3. Supabase のクライアント用変数のみ`VITE_`で始まっているか確認
4. R2 の秘密変数に`VITE_`が付いていないか確認
5. 値が正しく設定されているか確認
