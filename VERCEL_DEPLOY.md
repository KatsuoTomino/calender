...
- ✅ Cloudflare R2 環境変数（サーバー専用アクセスキー、バケット名）
...
| `R2_ENDPOINT`            | 任意: R2 S3 互換エンドポイント     |

`R2_*` は Vercel Function だけが参照するサーバー専用変数です。`VITE_R2_*` として登録しないでください。
...
5. 同様に `R2_*` 変数を追加（`VITE_` プレフィックスは付けない）
...
4. R2 の変数は `VITE_` ではなく `R2_` で登録されているか確認
5. 値が正しく設定されているか確認
