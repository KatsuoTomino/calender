...
3. **Add**をクリック
4. 同様に`VITE_SUPABASE_ANON_KEY`と R2 用のサーバー環境変数を追加

R2 の Access Key / Secret はブラウザに公開されないよう、必ず `VITE_` を付けずに設定してください。
...
3. Supabase の公開用変数は`VITE_`で始まり、R2 の秘密変数は`VITE_`で始まっていないことを確認
4. 値が正しく設定されているか確認
