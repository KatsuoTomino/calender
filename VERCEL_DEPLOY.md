...
以下の環境変数を準備してください。R2 の認証情報はサーバー関数のみで使うため、`VITE_` を付けない名前で設定してください。
...
> 注意: R2 のシークレットを `VITE_R2_SECRET_ACCESS_KEY` のような `VITE_` 付き変数としてクライアントに公開しないでください。
...
3. Supabase の公開変数は`VITE_`で始まり、R2 の秘密変数は`VITE_`なしで設定されているか確認
4. 値が正しく設定されているか確認
