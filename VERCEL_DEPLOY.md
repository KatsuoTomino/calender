...
# 任意: R2_ENDPOINT=https://<account-id>.r2.cloudflarestorage.com
```
...
R2のアクセスキーとシークレットには`VITE_`を付けないでください。Viteでは`VITE_*`環境変数がブラウザ向けバンドルに公開されます。
...
3. Supabaseの公開設定は`VITE_`で始まり、R2の秘密情報は`R2_`で始まっているか確認
4. 値が正しく設定されているか確認
