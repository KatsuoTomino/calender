    ...
    <div className="bg-white rounded-3xl shadow-sm p-4 h-full flex flex-col">
      ...
      <div className="flex items-center justify-between mb-4 px-2 shrink-0">
        <h2 className="text-2xl font-bold text-slate-800 flex items-baseline gap-2">
          ...
          <span className="text-primary text-3xl">{currentDate.getMonth() + 1}月</span>
        </h2>
      ...
      <div className="grid grid-cols-7 mb-2 shrink-0">
        {WEEKDAYS.map((day, idx) => (
          <div key={day} className={`text-center text-xs font-bold ${idx === 0 ? 'text-red-400' : idx === 6 ? 'text-blue-400' : 'text-slate-400'}`}>
            {day}
          </div>
        ))}
      </div>
