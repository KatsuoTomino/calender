...
import { generateSubtasks, generateEncouragement } from '../services/geminiService';
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar">
        {sortedTodos.length === 0 ? (
          ...
          sortedTodos.map(todo => (
            <div 
              key={todo.id} 
              className={`group flex items-center gap-3 p-3 rounded-xl border transition-all duration-200 ${todo.completed ? 'bg-slate-50 border-slate-100 opacity-60' : 'bg-white border-slate-100 shadow-sm hover:border-pink-200'}`}
            >
              <button 
                onClick={() => handleCheck(todo.id)}
                className={`flex-shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${todo.completed ? 'bg-secondary border-secondary' : 'border-slate-300 hover:border-secondary'}`}
              >
