...
import { monthBounds } from "../utils/todoDates";
...
export async function deleteMonthTodos(
  ...
  try {
    const { start: startDateStr, end: endDateStr } = monthBounds(year, month);
    ...
    if (todoIds) {
      ...
      query = query.gte("date_str", startDateStr).lte("date_str", endDateStr);
    }
    ...
    if (todoIds && (data?.length || 0) !== todoIds.length) {
      console.error("❌ 月のTodo削除エラー: 削除対象件数が一致しません");
      return false;
    }
