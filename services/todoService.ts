...
import { uniqueTodoIds } from "../utils/todoDates";
...
export async function addTodo(todo: TodoItem): Promise<boolean> {
  try {
    ...
    const { data, error } = await supabase.from("todos").insert(insertData).select("id");
...
export async function deleteMonthTodos(todoIds: string[]): Promise<boolean> {
  try {
    const ids = uniqueTodoIds(todoIds);
    if (ids.length === 0) {
      return true;
    }

    console.log(
      `🗑️ 月のTodoを削除中... (${ids.length}件)`
    );
