...
export async function deleteMonthTodos(
  ...
  try {
    ...
    const query = supabase.from("todos").delete().select("id");
    const { data, error } = todoIds
      ? await query.in("id", todoIds)
      : await query.gte("date_str", startDateStr).lte("date_str", endDateStr);
    ...
    if (todoIds && data?.length !== todoIds.length) {
      console.error("❌ 月のTodo削除件数が一致しません:", {
