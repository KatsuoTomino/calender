...
// 指定されたTodoを一括削除し、実際に削除できたIDを返す
export async function deleteMonthTodos(ids: string[]): Promise<string[] | null> {
  try {
    if (ids.length === 0) {
      return [];
    }

    console.log(`🗑️ ${ids.length}件のTodoを削除中...`);
    ...
    console.log("✅ 月のTodo削除成功:", deletedIds.length);
    return deletedIds;
  } catch (err) {
    console.error("❌ 予期しないエラー:", err);
    return null;
